...
const BASE_SUBDOMAIN: string | undefined = process.env.BASE_SUBDOMAIN;
...
const getData = async (url: URL): Promise<ApiVersion> => {
  const res = await got.get(url).json();
  if (isApiVersion(res)) return res;
  throw new Error("invalid response type recieved");
};

const getUrlPath = (env: envKeys, api: string): string => {
  return env === "tst" && api === "stocks" ? url : urlRoute[env];
};

const createUrl = (env: envKeys, api: string): URL => {
  const urlPath = getUrlPath(env, api);
  return new URL(`${PROTOCOL}://${env}${BASE_SUBDOMAIN}/${api}/${urlPath}`);
};
 ...
 */
export const printApiVersion = async (
  envSet: Set<string>,
  apiSet: Set<string>
) => {
  if (BASE_SUBDOMAIN === undefined)
    throw new Error(
      "BASE_SUBDOMAIN not found, pleaser make sure your .env is correctly set"
    );
